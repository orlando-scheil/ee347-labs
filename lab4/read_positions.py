...
try:
    ...
    print("Connected. Press Enter to read position, 'c' to toggle gripper (^C to exit)...")
    while True:
        line = sys.stdin.readline().strip().lower()
        if line == "c":
            gripper_open = not gripper_open
            val = 100 if gripper_open else 80
        ...
        if coords and angles:
            ...
            if within_bounds(row, MIN_ANGLES, MAX_ANGLES):
                ...
                print(f"  [{len(collected) - 1}] angles: {angles}  |  coords: {coords}")
